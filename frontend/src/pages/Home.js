...
const Home = () => {
  ...
  return (
    <div className="home-redesign">
      ...
      {/* Boutique Mosaic Grid */}
      <section className="boutique-mosaic-section">
        <div className="mosaic-header">
          <span className="mosaic-label">The Heritage Collective</span>
          <h2 className="brand-font-noble">Our Collections</h2>
        ...
        <div className="mosaic-grid">
          {categories.map((category, index) => (
            <div
              key={index}
              className={`mosaic-card card-variant-${(index % 3) + 1}`}
              onClick={() => navigate(`/shop?category=${encodeURIComponent(category.categoryValue)}`)}
            >
              <div className="mosaic-card-inner">
                <img src={category.image} alt={category.title} className="mosaic-img" />
                <div className="mosaic-overlay">
                  <div className="mosaic-content">
                    <h3 className="mosaic-title">{category.title}</h3>
                    <div className="mosaic-btn">
                      <span className="mosaic-btn-text">Discover</span>
                      <div className="mosaic-gold-dash"></div>
                    </div>
                  </div>
                </div>
              </div>
